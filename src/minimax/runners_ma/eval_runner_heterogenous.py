...
class EvalRunnerHeterogenous:
    ...
    def _get_transition(
            ...
            extra):
        _, _, pi_0_params, pi_1_params, next_actor_0_carry, next_actor_1_carry = self.pop.act(
            (actor_0_params, actor_1_params), obs, (actor_0_carry, actor_1_carry), done)

        pi_0 = self.pop.get_action_0_dist(pi_0_params, dtype=self.action_dtype)
        pi_1 = self.pop.get_action_1_dist(pi_1_params, dtype=self.action_dtype)
        rng, subrng = jax.random.split(rng)
        action_0 = pi_0.sample(seed=subrng)
        log_pi_0 = pi_0.log_prob(action_0)

        rng, subrng = jax.random.split(rng)
        action_1 = pi_1.sample(seed=subrng)
        log_pi_1 = pi_1.log_prob(action_1)
        ...
        if actor_0_carry is not None:
            next_actor_0_carry = jax.vmap(_tree_util.pytree_select)(
                done, zero_0_carry, next_actor_0_carry)

        if actor_1_carry is not None:
            next_actor_1_carry = jax.vmap(_tree_util.pytree_select)(
                done, zero_1_carry, next_actor_1_carry)
    ...
    def _rollout_benv(
        ...
        def _scan_rollout(scan_carry, rng):
            ...
            return (next_state, next_obs, next_0_carry, next_1_carry, extra, done, ep_stats), None
        ...
        (state,
         ...
         ep_stats), _ = jax.lax.scan(
            _scan_rollout,
            (state, obs, carry_0, carry_1, extra, done, ep_stats),
            rngs,
            length=n_steps)
    ...
    def run(self, rng, params_0, params_1):
        """
        ...
        """
        eval_stats = self.fake_run(
            rng, params_0)  # Params do not matter for the fake run
        ...
        for i, (benv, env_param) in enumerate(zip(self.benvs, self.env_params)):
            ...
            if self.pop.agent_0.is_recurrent:
                rng, subrng = jax.random.split(rng)
                actor_0_zero_carry, _ = self.pop.init_carry_agent_0(
                    subrng, obs['agent_0'])
            else:
                actor_0_zero_carry = None

            if self.pop.agent_1.is_recurrent:
                rng, subrng = jax.random.split(rng)
                actor_1_zero_carry, _ = self.pop.init_carry_agent_1(
                    subrng, obs['agent_1'])
            else:
                actor_1_zero_carry = None
            ...
            ep_stats = self._rollout_benv(
                rollout_rngs[i],
                benv,
                jax.lax.stop_gradient(params_0),
                jax.lax.stop_gradient(params_1),
                env_param,
                state,
                obs,
                actor_0_zero_carry,
